...
u8 CAN1_Mode_Init(u8 tsjw,u8 tbs2,u8 tbs1,u16 brp,u8 mode)
{
    ...
    GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化PA11,PA12
  	...
  	CAN_FilterInitStructure.CAN_FilterIdHigh= CAN1_TEST_STDID<<5;////32位ID
  	...
  	CAN_FilterInit(&CAN_FilterInitStructure);//滤波器初始化
		...
		CAN_FilterInitStructure.CAN_FilterIdHigh= CAN2_TEST_STDID<<5;////32位ID
		CAN_FilterInitStructure.CAN_FilterIdLow=0x0000;
		CAN_FilterInitStructure.CAN_FilterMaskIdHigh=0;
		CAN_FilterInitStructure.CAN_FilterMaskIdLow=0;
...
{	
  ...
  u16 i=0;
  CanTxMsg TxMessage;
  TxMessage.StdId=CAN1_TEST_STDID<<4;	 // 标准标识符为0
...
{	
  ...
  u16 i=0;
  CanTxMsg TxMessage;
  TxMessage.StdId=CAN2_TEST_STDID<<4;	 // 标准标识符为0
  TxMessage.ExtId=0x00;	 // 设置扩展标示符（29位）
  TxMessage.IDE=0;		  // 使用扩展标识符
  TxMessage.RTR=0;		  // 消息类型为数据帧，一帧8位
  TxMessage.DLC=len;							 // 发送两帧信息
